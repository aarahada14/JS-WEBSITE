...
function jewellery(){
  let newData = data.filter((ele) => ele.category == "jewelery")
  showRow(newData)
}

function men(){
  let newData = data.filter((ele) => ele.category == "men's clothing")
  showRow(newData)
}

function women(){
  let newData = data.filter((ele) => ele.category == "women's clothing")
  showRow(newData)
}
    ...
    data.map((ele) => {
        ...
        <div class="col-lg-4 col-md-6 col-sm-12">
            <div class="card h-100 bg-secondary-subtle">
                  <img src=${ele.image} height="250px" class="card-img-top  shadow  border border-2" alt="...">
                  <div class="card-body">
                      <h5 class="card-title">${ele.title}</h5>
                      <div class="d-flex justify-content-between">
                          <p class="card-text fs-5">${ele.category}</p>
                          <span class="fs-5">⭐ ${ele.rating.rate}</span>    
                      </div> 
                      <div class="d-flex justify-content-between">
                          <b class="">$ ${ele.price}</b>
                         
                          <span class="badge ${ele.rating.count< 120 ? `text-bg-danger` : ele.rating.count < 200 ? `text-bg-warning` : `text-bg-success`}"> ${ele.rating.count} 
                          <span class="badge text-bg-light p-0 px-1"> ${ele.rating.count< 200 ? `limited` : ``}</span></span>    
                      </div>
                       <hr>
